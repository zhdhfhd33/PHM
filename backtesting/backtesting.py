...
ILP_INTERVAL = 24   # ILP re-run interval (hours)
SEED = 42
# ──────────────────────────────────────────────────────


def run_backtesting():
    # 재현성: numpy뿐 아니라 torch(MC Dropout) 시드까지 고정해야 실행마다 동일 결과
    ...
    # 0. Load LSTM model (필수 - mock 모드 제거됨).
    #    SimulationEngine은 model_wrapper 없이는 동작하지 않는다.
    model_wrapper = ModelWrapper()
    print(f"  [OK] LSTM model loaded (RUL 예측 / 정비 후 새 베어링 RUL 예측)")

    # 1. Initialize PHM and B1
    #    모든 엔진이 각 AGV의 시나리오 RUL[0](데이터 기반 ground truth)에서 초기 RUL을
    #    직접 도출하므로, 동일 시나리오 파일을 읽는 한 ground truth는 자동으로 일치한다.
    phm_engine = SimulationEngine(num_agvs=N_AGV, K=K_SLOTS, model_wrapper=model_wrapper)

    b1_engine = BaselineSimulator(num_agvs=N_AGV, mode='NO_MAINT', K=K_SLOTS)
    # 2. Prepare B2 candidates
    tbm_candidates = [200, 500, 1000, 1500, 2000, 2500, 3000]
    b2_engines = {f"B2 TBM ({interval}h)": BaselineSimulator(num_agvs=N_AGV, mode='TBM', tbm_interval=interval, K=K_SLOTS)
                  for interval in tbm_candidates}

    # 3. Prepare B3 Pred-CBM candidates
    b3_thresholds = [24, 48, 96, 192]
    b3_engines = {
        f"B3 Pred-CBM ({thr}h)": SimulationEngine(num_agvs=N_AGV, K=K_SLOTS,
    ...
    for t in range(N_STEPS):
        ...
        if (t + 1) % 1000 == 0:
            ...
            progress_pct = ((t + 1) / N_STEPS) * 100
            elapsed_seconds = int(elapsed.total_seconds())

            # Calculate ETA
            if t + 1 > 0:
                avg_time_per_step = elapsed_seconds / (t + 1)
            ...
            print(f"  [Simulation] Step {t+1:5d}/{N_STEPS} ({progress_pct:5.1f}%) - Elapsed: {elapsed_str} - ETA: {eta_str}")

        # PHM ILP scheduler: run every 24h (Rolling Horizon T=168h)
        # T=168 makes ILP aware of failures up to 1 week ahead,
        # but only the first 24h of the schedule is enacted before re-optimizing.
        if phm_engine.current_time % ILP_INTERVAL == 0:
            ilp_run_count += 1
            ilp_progress_pct = (ilp_run_count / (N_STEPS // ILP_INTERVAL)) * 100
            print(f"  [ILP] Run {ilp_run_count:3d}/{N_STEPS // ILP_INTERVAL} ({ilp_progress_pct:5.1f}%) - Maintenance schedule computed")

            rul_samples = {agv.id: agv.rul_samples for agv in phm_engine.agvs}

            in_progress = phm_engine.get_in_progress_dict()
            scenario_dfs = phm_engine.get_scenario_dfs()

            schedule, _ = solve_maintenance_schedule(
                rul_samples, in_progress=in_progress, N_AGV=N_AGV, K=K_SLOTS,
                T=168,
                new_bearing_rul_samples=phm_engine.new_bearing_rul_samples
            )
            # Rolling Horizon: only enact schedules within the current 24h window.
            # ILP plans 168h ahead but only today's slice is acted upon;
            # tomorrow's re-optimization uses fresh RUL estimates.
            schedule_24h = {k: v for k, v in schedule.items()
                            if v is not None and v < ILP_INTERVAL}
            phm_engine.set_schedule(schedule_24h)

            # B3 Pred-CBM: threshold-based scheduling (no ILP)
    ...
    ax.plot(time_axis, cost_histories['PHM'], label='PHM (ILP Opt)', color='#2563EB', linewidth=3, zorder=10)
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"- **최적화 주기**: {ILP_INTERVAL}시간\n")
        ...
        f.write("- **✅ PHM-ILP**: LSTM 예측 불확실성을 고려한 수리적 최적 스케줄링\n")
    ...
    elapsed_str = f"{total_seconds // 3600:02d}:{(total_seconds % 3600) // 60:02d}:{total_seconds % 60:02d}"

    print(f"\n{'=' * 70}")
    # 콘솔 print에 이모지 금지: cp949 콘솔(한국어 Windows)에서 UnicodeEncodeError 발생
    print(f"  [OK] Simulation Completed")
    print(f"  End time: {sim_end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Total elapsed: {elapsed_str}")
    print(f"  Total cost: {phm_metrics['cost']:,} (10k KRW)")
    print(f"  Cost savings vs B1: {phm_metrics.get('savings_rate_vs_b1', 0):.1f}%")
    print(f"{'=' * 70}\n")
