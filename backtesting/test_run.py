...
def run_backtesting():
    # 재현성: numpy + torch(MC Dropout) 시드 고정
    ...
    # 0. Load LSTM model (필수)
    model_wrapper = ModelWrapper()

    # 1. Initialize PHM and B1
    #    모든 엔진이 각 AGV 시나리오 RUL[0]에서 ground truth를 직접 도출(자동 동일 함대)
    ...
    b3_engines = {
        f"B3 Pred-CBM ({thr}h)": SimulationEngine(num_agvs=N_AGV, K=K_SLOTS, model_wrapper=model_wrapper)
