...
전략:
  B1 NO_MAINT  : 예방 정비 없이 운영하다 고장 발생 시에만 사후(reactive) 긴급정비
                 (예측 미사용 — ground truth RUL이 0에 도달하면 고장)
  B2 TBM       : 고정 주기(tbm_interval h)마다 예방 정비 (전통적 TBM, 예측 미사용)

설계 메모:
  B1/B2는 예측 모델 없이 동작하므로 RUL을 데이터 기반 선형 카운트다운(ground truth)으로
  다룬다. 초기 RUL은 SimulationEngine과 동일하게 각 AGV의 시나리오 RUL[0]에서 직접
  도출하므로(engine.scenario_initial_rul), 모든 전략이 같은 고장 시각표 위에서 정책만
  달리 비교된다. RUL을 임의로 지정하는 통로는 없다.

공통 가정:
  - 초기 RUL = 각 AGV 시나리오 RUL[0] (PHM/B3와 동일 ground truth)
...
from src.simulation.engine import scenario_initial_rul
...
class BaselineSimulator:
    ...
    초기 RUL은 각 AGV의 시나리오 RUL[0]에서 도출한다(임의 지정 불가).
    ...
    def _reset(self):
        n = self.num_agvs
        # 초기 RUL = 각 AGV 시나리오 RUL[0] (데이터 기반 ground truth, PHM/B3와 동일)
        self.ruls = [scenario_initial_rul(i) for i in range(n)]
        # 각 AGV 베어링의 ground-truth 수명(정비 후 새 베어링 리셋값).
        # SimulationEngine과 동일하게 per-AGV 수명으로 리셋해 ground truth를 일관 유지.
        self._bearing_life = list(self.ruls)
        ...
        for i in range(self.num_agvs):
            if self.states[i] == 'MAINTENANCE':
                ...
                if self.maintenance_left[i] <= 0:
                    ...
                    # 정비 후 새 베어링 ground-truth 수명 리셋 (해당 AGV 베어링 수명).
                    # SimulationEngine._reset_to_new_bearing(시나리오 RUL[0])과 동일 가정.
                    self.ruls[i] = self._bearing_life[i]
