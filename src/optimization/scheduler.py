"""
ILP 기반 AGV 유지보수 스케줄러

계획서 Layer 3 정식화:
  결정 변수 : x[i, t] ∈ {0, 1}  — AGV i의 정비를 시각 t에 시작할지 여부
  계획 지평 : T (시간 단위 이산 슬롯). 운영 구성은 T=168h(1주) 롤링 호라이즌이며,
              매 24시간마다 재최적화하고 그중 첫 24h 구간의 스케줄만 실제로 enact한다
              (backtesting/app은 T=168로 호출). 함수 기본값 T=24는 단위 테스트용.
...
목적 함수 계수:
  coeff[i, t] = C_planned + (C_failure - C_planned)*P_fail[i][t]
                + C_failure*p_fail_after_maint(i, t)      # 정비 후 새 베어링 잔여 지평 고장
                - C_failure * P_fail[i][T]

  - (C_failure - C_planned)*P_fail[i][t] 항이 "정비 시각 t 이전 고장 리스크"를 반영해
    조기 정비를 유인한다(t가 커질수록 coeff 증가 → 솔버가 이른 t 선호).
...
def solve_maintenance_schedule(
    ...
    new_bearing_rul_samples=None,  # LSTM으로 예측한 새 베어링 RUL 샘플
):
    """AGV 유지보수 최적 스케줄 계산 (ILP / CBC 솔버).
    ...
    Args:
        ...
        model_wrapper: LSTM 모델 (정비 후 새 베어링 RUL 예측용, 선택사항)
        scenario_dfs : {agv_id: scenario_df} (새 베어링 초기 시나리오, 선택사항)
    ...
    Note:
        ...
        정비 후 새 베어링의 고장 확률은 LSTM으로 예측된 초기 RUL 샘플로부터 계산됩니다.
    """
    ...
    coeff = calculate_coefficients(
        P_fail, N_AGV, T_start, T, C_planned, C_failure,
        new_bearing_rul_samples=new_bearing_rul_samples,
        d=d
    )

    # 2. 최적화 문제 정의 (최소화)
    prob = pulp.LpProblem("AGV_Maintenance_Scheduling_24h_ILP", pulp.LpMinimize)
