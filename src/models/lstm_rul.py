...
def train_model(epochs: int = 150, sequence_length: int = 10,
                test_ratio: float = 0.2, batch_size: int = 64,
                lr: float = 1e-3):
    """LSTM RUL 모델 학습.

    sequence_length=10: 운영(ModelWrapper.seq_len=10)과 정합. 실측에서도 짧은
    윈도우가 점 추정 우위 (agent_log/B안_파이프라인_근본수정/03 참고).
